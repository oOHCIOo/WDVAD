...
def anomap(predict_dict, label_dict, length_dict, save_path, round,  zip=False):
    """
    ...
    :return:
    """
    plot_dir = os.path.join(save_path, 'logs', 'plot')
    if not os.path.exists(plot_dir):
        os.makedirs(plot_dir)

    if zip:
        zip_file_name = os.path.join(plot_dir, f'round_{round}.zip')
        with zipfile.ZipFile(zip_file_name, mode="w") as zf:
            for k, v in predict_dict.items():
                ...
                img_name = k + '.jpg'
                predict_np = v.repeat(16)[:valid_length]
                
                frame_labels = label_dict.get(k)
                if frame_labels is not None:
                    label_np = frame_labels[:valid_length]
                else:
                    label_np = np.zeros(valid_length)
        ...
        round_plot_dir = os.path.join(plot_dir, f'round_{round}')
        os.makedirs(round_plot_dir, exist_ok=True)
        for k, v in predict_dict.items():
            valid_length = length_dict.get(k)
            if valid_length is None:
                continue
            ...
            plt.grid(True, linestyle='-.')
            plt.legend()
            plt.savefig(os.path.join(round_plot_dir, k))
            plt.close()
