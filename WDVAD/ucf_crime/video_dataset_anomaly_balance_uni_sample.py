...
class dataset(Dataset):
    def __init__(self, args,new_video_filename=None,dataset_name="default_dataset", train=True, trainlist=None,updated_list=None, testlist=None):
        """
        :param args:
        self.dataset_type: path to dir contains anomaly datasets
        ...
        if self.feature_pretrain_model == 'c3d' or self.feature_pretrain_model == 'c3d_ucf':
            ...
            self.feature_path = os.path.join(self.cur_path, f'{self.dataset_type}dataset' ,'features_video',
                                             self.feature_pretrain_model, self.feature_layer, self.feature_modal)
        elif self.feature_pretrain_model == 'i3d':
            self.feature_path = os.path.join(self.cur_path,f'{self.dataset_type}dataset' , f'features_video', self.feature_pretrain_model, self.feature_modal)

        self.videoname = os.listdir(self.feature_path)
        self.data_dict = self.data_dict_creater()
        if updated_list:
            self.updated_list = self.txt2list(txtpath=os.path.join(self.cur_path, f'{self.dataset_type}dataset' , self.dataset_name,updated_list))

        if trainlist:
            self.trainlist = self.txt2list(txtpath=os.path.join(self.cur_path, f'{self.dataset_type}dataset' , self.dataset_name,trainlist))
        else:
            self.trainlist = self.txt2list(txtpath=os.path.join(self.cur_path, f'{self.dataset_type}dataset' , self.dataset_name, f'{dataset_name}_train.txt'))
            
        if testlist:
            self.testlist = self.txt2list(txtpath=os.path.join(self.cur_path, f'{self.dataset_type}dataset' , self.dataset_name,testlist))
        else:
            self.testlist = self.txt2list(txtpath=os.path.join(self.cur_path, f'{self.dataset_type}dataset' , self.dataset_name, f'{dataset_name}_test.txt'))
        ...
        if new_video_filename is not None:
            ...
            self.video_label_dict = self.pickle_reader(
                file=os.path.join(self.cur_path, f'{self.dataset_type}dataset' ,  'GT', 'video_label.pickle'))
            self.frame_label_dict = self.pickle_reader(file=os.path.join(self.cur_path, f'{self.dataset_type}dataset' , 'GT', 'frame_label.pickle'))
        if updated_list:
            self.updated_list = self.txt2list(txtpath=os.path.join(self.cur_path, f'{self.dataset_type}dataset' , self.dataset_name, 'updated_samples.txt'))
    ...
    def p_n_split_dataset(self, video_label_dict, trainlist):
        ...
        for t in trainlist:
            clean_t = t.strip()
            if video_label_dict.get(clean_t, [0.0]) == [1.0]:
                anomaly_video_train.append(clean_t)
            else:
                normal_video_train.append(clean_t)
    ...
    def __getitem__(self, index):
        ...
        if self.args.larger_mem: #0
            if self.train:
                train_video_name = []
                start_index = []
                anomaly_indexs = random.choices(self.anomaly_video_train, k=self.args.sample_size)
                normaly_indexs = random.choices(self.normal_video_train, k=self.args.sample_size)
            ...
            if self.train:
                ...
                # 【新增】从加载好的字典中查找原始帧数
                if data_video_name in self.frame_label_dict:
                    original_frame_count = len(self.frame_label_dict[data_video_name])
                else:
                    # print(f"信息 (Dataset): 视频 '{data_video_name}' 标签缺失。使用特征长度估算其帧数。")
                    original_frame_count = self.feature.shape[0] * 16
                # return self.feature, data_video_name
                return torch.from_numpy(self.feature), data_video_name, original_frame_count
