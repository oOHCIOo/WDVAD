...
def anomap(predict_dict, label_dict, save_path, round,  zip=False):
    """
    ...
    :return:
    """
    if os.path.exists(os.path.join( save_path, 'logs','plot')) == 0:
        os.makedirs(os.path.join( save_path, 'logs','plot'))
    if zip:
        zip_file_name = os.path.join( save_path, 'plot','logs', 'round_{}.zip'.format(round))
        ...
        for k, v in predict_dict.items():
            ...
            if len(predict_np) != len(label_np):
                print(f'{k}_{len(predict_np)}_{len(label_np)}')
            x = np.arange(len(predict_np))
            ...
            plt.legend()
            # plt.show()
            if os.path.exists(os.path.join(save_path, 'logs','plot', 'round_{}'.format(round))) == 0:
                os.makedirs(os.path.join( save_path, 'logs','plot', 'round_{}'.format(round)))
                plt.savefig(os.path.join( save_path, 'logs','plot', 'round_{}'.format(round), k))
            else:
                plt.savefig(os.path.join(save_path, 'logs','plot', 'round_{}'.format(round), k))
            plt.close()
