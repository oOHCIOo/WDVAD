...
使用方法：
    python ablation_study.py --cid 2 --dataset_type shanghaitech --pvalue_thresholds 0.01 0.05 0.1 0.15 0.2

或者使用默认的pvalue阈值范围：
    python ablation_study.py --cid 2 --dataset_type shanghaitech

或者不使用分数下降条件（仅基于pvalue阈值）：
    python ablation_study.py --cid 2 --dataset_type shanghaitech --no_score_decrease
"""
...
def main():
    ...
    # Update args.dataset_type to ensure consistency
    args.dataset_type = dataset_type
    
    # Determine dataset name based on cid (format: dataset_type_cid)
    dataset_name = f"{dataset_type}_{args.cid}"
    
    # Print dataset information for debugging
    print(f"\n消融实验配置:")
    print(f"  数据集类型: {dataset_type}")
    print(f"  数据集名称: {dataset_name}")
    print(f"  客户端ID: {args.cid}")
    ...
    if ablation_args.pvalue_thresholds:
        ...
        print("使用默认pvalue阈值范围: [0.001, 0.005, 0.01, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1.0]")
...
# 使用示例：
# python ablation_study.py --cid 2 --dataset_type shanghaitech
# python ablation_study.py --cid 2 --dataset_type shanghaitech --pvalue_thresholds 0.01 0.05 0.1
# python ablation_study.py --cid 2 --dataset_type shanghaitech --no_score_decrease
